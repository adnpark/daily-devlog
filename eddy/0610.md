...
DataSource는 모델이 아닌가? 라는 의문이 들었다. 데이터 로딩하는 모델 객체가 직접 UICollectionViewDataSource를 채택한다면 편하지 않을까?

(문제 해결) Struct 객체에서 비동기 데이터 불러오기

모델을 Struct로 만들었다. Struct로 만든 이유는 00이었다. 그런데 다음과 같은 에러가 발생했다.
