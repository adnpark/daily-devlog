...
그런데 이렇게 했으면, 실제로 생성할 때 객체를 만들어줘야하는데... 그건 어디서 해야할까? 실제로 주입을 해주는 곳은 어디일까?

2. DI Container로 의존성 주입하기

실제로 주입을 해주는 곳은, DS를 사용하는 객체인 상위 VC가 될 것이다. 하지만 이렇게 되면 상위 VC가 DS의 의존 관계를 다 알아야 한다. 

그래서 DI Container가 등장한다.
